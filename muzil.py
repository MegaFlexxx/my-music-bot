...
async def download_and_send(message: types.Message, track_id: str):
    ...
    try:
        ...
        if cover_url:
            try:
                with open(cover_name, 'wb') as f: f.write(requests.get("https:" + cover_url if not cover_url.startswith("http") else cover_url, timeout=5).content)
